...
**Allowlist（用户白名单，用户管理的永久放行）**：用户在面板 `Allowlist Skills` 区域手动添加；命中后绕过 ModeGate（**含 Delete / PlayMode / Reload / `RiskLevel="high"` 等 NeverInSemi 拦截**），但**不绕过**高危 ConfirmationToken 二次确认（若 `RequireConfirmation` 开启，仍需带 `_confirm` token 重放）；AI 一般不应主动 add/remove。
